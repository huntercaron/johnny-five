  ...
  TCS34725: {
    ...
    initialize: {
      value: function(opts, dataHandler) {
        ...
        this.io.i2cReadOnce(address, command(this.REGISTER.ID), 8, function(bytes) {
          console.log(bytes);
        });

        this.io.i2cWriteReg(address, command(this.REGISTER.ATIME), 0xF6);
        this.io.i2cWriteReg(address, command(this.REGISTER.CONTROL), 0x01);
        this.io.i2cWriteReg(address, command(this.REGISTER.ENABLE), this.REGISTER.ENABLE_PON);


        setTimeout(() => {
          this.io.i2cWriteReg(address, command(this.REGISTER.ENABLE), this.REGISTER.ENABLE_PON | this.REGISTER.ENABLE_AEN);
        }, 6);




        const readColor = colorAddress => new Promise(resolve => this.io.i2cReadOnce(address, command(colorAddress), 4, (bytes) => resolve(bytes[0]+(bytes[1]*255)) ));


        setInterval(() => {
          let clear = readColor(this.REGISTER.CDATAL);
          let red = readColor(this.REGISTER.RDATAL);
          let green = readColor(this.REGISTER.GDATAL);
          let blue = readColor(this.REGISTER.GDATAL);

          let value = "";

          Promise.all([clear,red,green,blue]).then(values => {
            let clear = values[0];

            for (let i = 1; i < 4; i++) {
              let calc = Math.floor((values[i]/clear)*255);

              value += pad(calc.toString(16), 2);
            }
