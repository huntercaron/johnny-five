  ...
  TCS34725: {
    ...
    initialize: {
      value: function(opts, dataHandler) {
        ...
        const readColor = colorAddress => new Promise(resolve => this.io.i2cReadOnce(address, command(colorAddress), 4, (bytes) => resolve(bytes[0]) ));
        ...
        setInterval(() => {
          ...
          Promise.all([red,green,blue]).then(values => {
            for (var i = 0; i < 3; i++) {
              value += pad(values[i].toString(16), 2);
            }
